...
function drawSkeletons(ctx, skeletons, normalizer) {
    skeletons.forEach(function (skeleton, i) {
        var vis = $('#visibility-button' + i);
        if (vis.hasClass('selected')) {
            return;
        }
        ctx.beginPath();
        var p1 = normalizer(skeleton.a);
        ctx.moveTo(p1.x, p1.y);
        var p2 = normalizer(skeleton.b);
        ctx.lineTo(p2.x, p2.y);
        ctx.setLineDash([4, 4]);
        var btn = $('#skeleton-button' + i);
        if (btn.hasClass('selected') || btn.hasClass('axis')) {
            ctx.strokeStyle = btn.hasClass('selected') ? 'green' : 'red';
            ctx.lineWidth = btn.hasClass('selected') ? 1 : 3;
...
function generateNormalizer(state, height) {
    ...
    */
    state.skeletons.forEach(function (s) {
        minX = Math.min(minX, s.a.x);
        minX = Math.min(minX, s.b.x);
        minY = Math.min(minY, s.a.y);
        minY = Math.min(minY, s.b.y);
    });
...
function updateSkeletonList(skeletons) {
    var visibilityListDiv = $('#visibility-list');
    var skeletonListDiv = $('#skeleton-list');
    skeletonListDiv.html('');
    visibilityListDiv.html('');
    for (var i = 0; i < skeletons.length; ++i) {
        visibilityListDiv.append('<button class="btn btn-default visibility-button" id="visibility-button' + i + '" data-skeleton-index="' + i + '">V' + i + '</button>');
    ...
    $('.visibility-button').click(function() {
        ...
        var cur = window.state.skeletons[parseInt(self.data('skeleton-index'))].visible;
        window.state.skeletons[parseInt(self.data('skeleton-index'))].visible = !cur;
        drawThings();
    });
    $('.skeleton-button').click(function () {
        ...
        if (self.hasClass('selected')) {
            ...
            self.addClass('axis');
        } else if (self.hasClass('axis')) {
            self.removeClass('axis');
        } else {
...
$(document).ready(function () {
    ...
    $('.submit-button').click(function () {
        var self = $(this);
        var phase = self.attr('id') == 'finish' ? 'end' : 'unfold';
        var unfoldIds = $.map($('.skeleton-button.selected'), function(e) {
            return $(e).data('skeleton-index');
        });
        var axisId = $('.skeleton-button.axis').data('skeleton-index');
        var history = $('#history').val();
        $.ajax({
            url: '/gekirin/burning/send',
            data: {
                unfoldIds: unfoldIds,
                axis: axisId,
                skeletons: window.state.skeletons,
                history: history,
                phase: phase
            },
            dataType: 'json',
