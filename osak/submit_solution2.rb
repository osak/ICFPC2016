...
def main
  ...
  stdin, stdout, stderr = Open3.popen3('/home/icfpc/shared/kawatea/shuffle')
  stdin.write(output)
  shuffled = stdout.read
  stdin.close
  stdout.close
  stderr.close

  stdin, stdout, stderr = Open3.popen3('/home/icfpc/shared/kawatea/count')
  stdin.write(shuffled)
  stdin.close
  sol_size = stdout.read.to_i
  stdout.close
  stderr.close
  if sol_size > 5000
    STDERR.puts("ファイルでかすぎ！！ size=#{sol_size}")
    exit 3
  end
  doc = {
