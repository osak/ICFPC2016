...
string solve(Polygon& p, Rational& xd, Rational& yd, Rational& xmin, Rational& xmax, Rational& ymin, Rational& ymax, const Matrix& m) {
    ...
    Rational cx = x / xd;
    if (cx.is_int()) {
        for (i = cx.to_int() - 1; i >= 1; i--) ps = cut(ps, Line(Point(xmin + xd * i, ymin), Point(xmin + xd * i, ymin + 1))).second;
    } else {
        while (xd * 2 <= x) {
            x /= 2;
    ...
    Rational cy = y / yd;
    if (cy.is_int()) {
        for (i = cy.to_int() - 1; i >= 1; i--) ps = cut(ps, Line(Point(xmin + 1, ymin + yd * i), Point(xmin, ymin + yd * i))).second;
    } else {
        while (yd * 2 <= y) {
            y /= 2;
