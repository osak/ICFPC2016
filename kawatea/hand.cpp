...
vector <Polygon> cut(vector <Polygon>& ps, const Line& l) {
    ...
    for (i = 0; i < ps.size(); i++) {
        ...
        for (j = 0; j < ps[i].v.size(); j++) cerr << ps[i].v[j] << endl;
        cerr << "Fold? (y/n)" << endl;
...
string solve(const string& filename) {
    ...
    {
        cerr << "Use original line? (y/n)" << endl;
        ...
        if (ans == "y") {
            ...
            has_line = true;
            start = Line(get_point(s), get_point(t));
        }
    }
    
    for (i = 0; i < m && has_line == false; i++) {
        for (j = 0; j < 2; j++) {
            if (j == 0) {
                start = skeleton[i];
            } else {
                start = Line(skeleton[i].p2, skeleton[i].p1);
            }
            
            cerr << "Use " << start.p1 << " " << start.p2 << " as origin? (y/n)" << endl;
    ...
    Rational d = dist(start.p1, start.p2);
    
    Rational x = (start.p2.x - start.p1.x) / d;
    Rational y = (start.p1.y - start.p2.y) / d;
    
    Matrix mat = Matrix(start.p1, x, y);
    
    for (i = 0; i < 4; i++) v[0].v[i] = mat.reconvert(v[0].v[i]);
    
    while (1) {
        cerr << "End folding? (y/n)" << endl;
        
        cin >> ans;
        
        if (ans == "y") return output(v, mat);
        ...
        for (i = 0; i < m; i++) {
            cerr << "Fold on " << skeleton[i].p1 << " " << skeleton[i].p2 << "? (left/right/n)" << endl;
            
            cin >> ans;
            
            if (ans != "l" && ans != "r" && ans != "left" && ans != "right") continue;
            ...
            if (ans == "l" || ans == "left") {
                ...
                l = Line(skeleton[i].p2, skeleton[i].p1);
            }
