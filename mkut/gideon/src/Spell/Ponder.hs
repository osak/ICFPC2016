...
cruise' :: Segment -> [Point] ->  Solution -> Solution
cruise' seg@((a, b), (c, d)) ps sol = Solution ss2 fs2 ds2
    where
        ss = sources sol
        fs = facets sol
        ds = destinations sol
        ...
        refl (x, y) = if cross (c - a, d - b) (x - s, y - t) > 0 || not (elem (x, y) ps) then (x, y) else (2 * s - x, 2 * t - y)
            where
                (s, t) = proj (x, y)
