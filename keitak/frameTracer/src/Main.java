...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        int N = sc.nextInt();
        Set<Point> silhouette = new HashSet<>();
        for (int i = 0; i < N; i++) {
            int M = sc.nextInt();
            for (int j = 0; j < M; j++) {
                String line = sc.next();
                silhouette.add(formPoint(line));
            }
        }
        ...
        for (int i = 0; i < M; i++) {
            String s = sc.next();
            String t = sc.next();
            edges.add(new Edge(formPoint(s), formPoint(t)));
        }
        Point points[] = new Point[silhouette.size()];
        points = silhouette.toArray(points);
        Edge[] es = getEdges(points);
    ...
    static Edge formEdge(String line) {
        String st[] = line.split("¥s");
        Point s = formPoint(st[0]);
        Point t = formPoint(st[1]);
        return new Edge(s, t);
    }
