...
vector<P> interpolate(vector<P> &src, vector<P> &dst, Rational x) {
    int n = src.size();
    vector<P> mid(n);
    for (int i = 0; i < n; ++i) {
        mid[i].first = src[i].first * (-x + 1) + dst[i].first * x;
        mid[i].second = src[i].second * (-x + 1) + dst[i].second * x;
    }
    return mid;
}

vector<vector<P>> getPolygons(vector<P> vp, vector<vector<int>> facets) {
    int k = facets.size();
    vector<vector<P>> pfacets(k);
    for (int i = 0; i < k; ++i) {
        for (int j = 0; j < facets[i].size(); ++j) {
            pfacets[i].push_back(vp[facets[i][j]]);
        }
    }
    return pfacets;
}
...
int main() {
    ...
    cout << "animate([" << endl;
    for (Rational r = 0; r <= 1; r += Rational(1, 100)) {
        if (r > 0) {
                cout << ",";
        }
        cout << "function() {" << endl;
        auto ps = interpolate(source, dest, r);
        auto poly = getPolygons(ps, facets);
