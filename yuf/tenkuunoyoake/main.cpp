...
bool outputProblem (const vector<Line> &skeleton, const Line mirrorLine, int ccw) {
#ifdef _DEBUG
    ...
    cout << "reflecting by the segment : " << mirrorLine.p1 << ' ' << mirrorLine.p2 << endl;
    cout << "start output problem....:" << endl;
    cout << 1 << endl << n << endl;
    for (int i = 0; i < n; ++i) {
    ...
    cout << "end output problem" << endl;
    if (ccw == LEFT) cout << "fold " << mirrorLine.p1 << ' ' << mirrorLine.p2 << endl;
    if (ccw == RIGHT) cout << "fold " << mirrorLine.p2 << ' ' << mirrorLine.p1 << endl;
#ifdef _DEBUG
    cout << "ccw " << ccw << endl;
...
//        }
        ...
        for (int i = 0; i < skeleton.size(); ++i) {
            if (i == pos) continue;
            if (used[i]) {
                newSkeleton.push_back(reflection(skeleton[pos], skeleton[i]));
...
//                output(skeleton[i]);
//                output(reflection(skeleton[pos], skeleton[i]));
//                cout << "end reflection" << endl;
            } else {
                newSkeleton.push_back(skeleton[i]);
            }
        }

        // TODO: check if it's invalid (e.g. self-cross) polygon

        // TODO: check if it will cross original figure

        // mirror-skeletons in path
        if (outputProblem(mergeSegments(newSkeleton), skeleton[pos], x)) {
            exit(0);
        }
...
//        output(skeleton[i]);
    ...
    // building a graph using skeletons
...
//            output(skeleton[j]);
    ...
    map<Point, int> vertices;
    int k = v.size();
    for (int i = 0; i < k; ++i) {
        vertices[v[i]] = i;
    }

    auto onOutline = [&](Line line) {
        if (!vertices.count(line.p1) || !vertices.count(line.p2)) return false;
        return abs(vertices[line.p1] - vertices[line.p2]) == 1
        || abs(vertices[line.p1] - vertices[line.p2]) == vertices.size() - 1;
    };

    for (int i = 0; i < m; ++i) {
        if (onOutline(skeleton[i])) {
#ifdef _DEBUG
            cout << "try to reflect with segment: ";
            cout << skeleton[i].p1 << ' ' << skeleton[i].p2 << endl;
#endif
